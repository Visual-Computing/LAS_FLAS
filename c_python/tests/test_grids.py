...
def test_grid_creations_list():
    def _create_grid_by_feature_list(n: int, dim: int, aspect_ratio: float = 1.0) -> Grid:
    ...
    for n in [1, 7, 25, 51, 60*60-1, 60*60]:
        for aspect_ratio in [1.0, 0.5, 2.0, 16 / 9]:
            _grid = _create_grid_by_feature_list(n, dim, aspect_ratio)


def test_grid_creations_map():
    def _create_grid_by_feature_map(height: int, width: int, dim: int) -> Grid:
    ...
    for height in [1, 7, 25, 51, 60]:
        for width in [1, 7, 25, 51, 60]:
            _grid = _create_grid_by_feature_map(height, width, dim)


def test_builder_dynamic():
    def _create_grid_dynamic_builder(n: int, dim: int, aspect_ratio: float = 1.0) -> Grid:
    ...
    for n in [1, 7, 25, 51, 60*60-1, 60*60]:
        for aspect_ratio in [1.0, 0.5, 2.0, 16 / 9]:
            _grid = _create_grid_dynamic_builder(n, dim, aspect_ratio)
